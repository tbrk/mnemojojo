...
public class Panel extends Container
{
	...
	protected int normalVScrollLength,fastVScrollLength,normalHScrollLength,fastHScrollLength;
	...
	public void paint(Graphics g)
	{
		...
		if (showDecorations) // this panel has decorations
		{
			...
			if (labelLeft != null) {
				g.drawString(labelLeft, 0, labelY, Graphics.TOP | Graphics.LEFT);
			}
		}
	...
	public void setLabel(String label)
	{
		...
		{// find the labelX and labelY.
			...
			if (tabIdx != -1) {
				labelLeft = label.substring(0, tabIdx);
				this.label = label.substring(tabIdx + 1);
			} else {
				labelLeft = null;
			}

			int strLen = f.stringWidth(this.label);
			int strHeight = f.getHeight();

			if (labelLeft != null) {
				align = FireScreen.RIGHT;
			}
	...
	protected void scroll(int gameCode, int pixels)
	{
		int vpx = getViewPortPositionX();
		if(gameCode==Canvas.LEFT)
		{
